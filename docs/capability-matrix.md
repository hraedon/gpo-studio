...
| 027 | `folder_redirection.py` | no | **capture-backed (R3)** — and the capture invalidates the module's *scope*: the policy lives in `fdeploy1.ini`, which the module does not address. Measured 2026-09-11: an advanced policy with three group rules yields one registry tuple, carrying neither the group SIDs nor the four option flags that R3 shows Windows encoding as `Flags=1021`. The CSE appears in 0 of the 26 production GPOs (R6). Awaiting a ruling — [the brief](scope-brief-2026-09-11-folder-redirection.md) recommends read-target, with write deferred, and says where the Software Installation precedent stops applying |
