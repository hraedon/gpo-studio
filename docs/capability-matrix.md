...
The Windows-verified column below is **three-valued**, added 2026-09-06 after
the eleven manual-evidence requests. The reason is that flat `no` had become
the wrong answer twice over: it said the same thing about `object_security.py`,
whose wire shape Windows' own parser has now ruled on, as it said about
`network_security.py`, which no oracle has ever read. That is a real loss of
information, and it invites the opposite error — someone noticing the
correction history and "fixing" the matrix by writing `yes`.
...
`capture-backed` is defined by what it **denies**, and every clause is load-bearing:
...
- It is **not partial credit toward promotion.** The exit condition in
  [`domain-layer-status.md`](domain-layer-status.md) stays exactly two-valued —
  a lane certification **and** a delivery surface. `capture-backed` is neither
  half of it and does not shorten the distance to either.
- It **does not cover the module**, only the facts cited. `security_template.py`
  is `capture-backed (R4)` for the encoding and section shape of one native
  template, and in the same breath that capture showed it parses none of that
  template's `[Registry Keys]` rows.
- It **requires a citation.** A `capture-backed` cell naming no request is not a
  weaker claim, it is an unsupported one, and
...
| 027 | `folder_redirection.py` | no | **capture-backed (R3)** — and the capture invalidated the module's *scope*: the policy lives in `fdeploy1.ini`, which this module does not address. Ruled 2026-09-11 a **read target** ([the decision](scope-decision-2026-09-11-folder-redirection.md)); the read half landed as `fdeploy.py`, reachable at `POST /api/folder-redirection/fdeploy` and through the [Folder Redirection browser review panel](folder-redirection-review.md), tested against the banked R3 capture and bound by no lane. `Flags` is carried and not decoded — one observation of a ten-bit word (WI-066) — so the writer is deferred until R12 measures the encoding. The parse reaches no `GPO`, so reports and diffs of an imported backup still show the file as a hash (WI-068). This row stays `no`: a module is a capability only with a delivery surface **and** Windows evidence, and no lane has read this artifact |
...
`"KEY",code,"SDDL"` — measured on a native template (R4) and confirmed by
`secedit /validate`, which rejects the `key = value` form there and accepts
this one (R9).

**`security_template.py` does not parse them, deliberately.** It is the generic
INF codec; it preserves these rows verbatim in `InfSection.unknown_lines`,
round-trips them byte-exact, reports whole-line `removed`/`added` pairs in
`diff_templates`, and raises an `unparsed_entries` warning naming the section
and line count. It does not offer `get_value`, semantic validation, or
entry-level diff for them. That is the `preserve-only` state as this matrix
defines it, and it is the honest description of the codec.

**It is not a statement about the product.** `object_security.py` parses these
rows in full — `key_path`, `propagation` (codes measured 0/1/2), and a parsed
`SecurityDescriptor` down to trustee SIDs and rights. The typed semantics live
in the family layer, which is where the types are; teaching the codec a second
entry grammar would give two modules an opinion about the same bytes.

Two limits on that, stated because the sentence above is the one most likely to
be over-read:
...
Nothing here has moved closer to the capability matrix proper.
