...
| 027 | `folder_redirection.py` | no | **capture-backed (R3)** — and the capture invalidated the module's *scope*: the policy lives in `fdeploy1.ini`, which this module does not address. Ruled 2026-09-11 a **read target** ([the decision](scope-decision-2026-09-11-folder-redirection.md)); the read half landed as `fdeploy.py`, reachable at `POST /api/folder-redirection/fdeploy`, tested against the banked R3 capture and bound by no lane. `Flags` is carried and not decoded — one observation of a ten-bit word (WI-066) — so the writer is deferred until R12 measures the encoding. The parse reaches no `GPO`, so reports and diffs of an imported backup still show the file as a hash (WI-068). This row stays `no`: a module is a capability only with a delivery surface **and** Windows evidence, and no lane has read this artifact |
