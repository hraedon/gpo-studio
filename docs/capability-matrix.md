...
| 025 | `network_security.py` | no | no |
| 025 | `policy_families.py` | no | **capture-backed (R7)** — `[Kerberos Policy]` key names and mixed units from a real DC. The three existing unit mappings were confirmed **correct**; two unmodelled keys added |
...
| 030 | `publication.py` | no | **capture-backed (R5, R8, R11)** — packed `gpt.ini` version arithmetic, corroborated independently in the lab and in a production directory |
