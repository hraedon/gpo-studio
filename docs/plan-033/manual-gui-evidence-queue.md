...
- [ ] RDP to `mvmcitest01` and open Group Policy Management.
- [ ] Create an unlinked GPO named `GPOStudio-WI022-Manual-<timestamp>`.
- [ ] Edit Computer Configuration > Preferences > Control Panel Settings >
  Services.
- [ ] Add an Update item for the built-in `Spooler` service.
- [ ] Set first failure to Run a Program, second failure to Restart the
  Service, and subsequent failure to Restart the Computer.
- [ ] Set reset-fail-count to 2 days and restart-service delay to 7 minutes.
- [ ] Set the recovery program to `C:\Windows\System32\cmd.exe`, arguments to
  `/c exit 0`, and enable the UI option that appends the failure count.
- [ ] Set restart-computer delay to 3 minutes and message to
  `Synthetic GPO Studio recovery evidence`.
- [ ] Add a second Update item for `W32Time`; select Local System and enable
  desktop interaction if GPMC permits that combination. Do not select or enter
  a named account or password.
- [ ] Save and close the editor, then take screenshots of both completed
  property dialogs with every recovery field visible.
- [ ] In an elevated Windows PowerShell 5.1 session, run the capture commands
...
- [ ] Tell the agent the exact GPO name and that capture is complete. The agent
