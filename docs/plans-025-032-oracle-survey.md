...
> **DECIDED 2026-09-06: `oracle_evidence.py` is.** `certification.py` is
> superseded — no consumer outside its own tests, no `inconclusive` or
> `unsupported` state, and `round_trip` as its default oracle. Enforced by
> `test_certification_module_has_no_production_consumer`; removal tracked as
> WI-056. See
...
> **DECIDED 2026-09-06: no.** Read-only/preserve-only; `.aas` generation is
> out of scope and the feasibility capture is **not** booked. The new input
> was R6: the Software Installation CSE appears in **0 of 26** production
> GPOs, and no sampled SYSVOL has an `Applications/` tree. Same decision
> document.
