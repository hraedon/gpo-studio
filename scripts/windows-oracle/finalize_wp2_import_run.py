...
def main() -> int:
    ...
    source_hashes = {name: _sha256(path) for name, path in source_map.items()}
    checks["harness_matches_source"] = all(
        (run_dir / name).is_file() and _sha256(run_dir / name) == digest
