...
# Retrieve the actual deployed harness files from the Windows host for
# provenance verification (not source-tree copies).  These are the bytes
# that executed on Windows, retrieved post-run.
...
# Locally-executed scripts: copy from the source tree (these ran locally,
# not on Windows, so the source-tree copy IS the executed copy).
