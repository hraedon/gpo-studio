#!/usr/bin/env bash
# Plan 033 WP-3 security-template conformance run.
#
...
#                 acb exec cred:svc-da -- bash scripts/windows-oracle/run-wp3-oracle.sh
#
...
#
# This harness never invokes `secedit /configure`, on either transport: it
# validates, imports into a temporary database, exports, and removes it. The
# estate is disposable and the restriction is no longer forced by a shared host,
# but the lane's claim is about template conformance, not about applying policy.
#
